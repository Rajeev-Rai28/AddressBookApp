...
import com.addressbookapp.storage.ContactStorage;
...
public class AddressBookService {
    ...
    public Contact addContact(String bookName, Contact contact) {
        AddressBook book = createAddressBook(bookName);

        for (Contact existing : book.getContacts()) {
            String existingFirst = existing.getFirstName() == null ? "" : existing.getFirstName().trim();
            String existingLast = existing.getLastName() == null ? "" : existing.getLastName().trim();

            String newFirst = contact.getFirstName() == null ? "" : contact.getFirstName().trim();
            String newLast = contact.getLastName() == null ? "" : contact.getLastName().trim();

            if (existingFirst.equalsIgnoreCase(newFirst) && existingLast.equalsIgnoreCase(newLast)) {
